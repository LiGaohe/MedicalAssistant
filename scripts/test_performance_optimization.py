"""
性能优化测试脚本

测试病历生成流程的性能优化效果，对比串行和并行模式的性能差异
"""
import sys
import os
import time

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from backend.database import SessionLocal, init_db
from backend.models import Visit, TranscriptTurn, LLMConfig
from backend.services.medical_record_pipeline import MedicalRecordPipeline
from backend.services.llm.llm_service import LLMService


def create_test_data(db):
    """创建测试数据"""
    visit = Visit(
        visit_id="perf_test_001",
        patient_name="测试患者",
        visit_date="2026-05-07",
        audio_path="test_audio.wav",
        status="transcribed"
    )
    db.add(visit)
    
    turns_data = [
        {"turn_index": 0, "speaker": "spk0", "text": "你好，请问哪里不舒服？", "start_ms": 0, "end_ms": 2000},
        {"turn_index": 1, "speaker": "spk1", "text": "医生，我这几天一直头疼，特别是早上起来的时候。", "start_ms": 2500, "end_ms": 5500},
        {"turn_index": 2, "speaker": "spk0", "text": "头疼持续多长时间了？", "start_ms": 6000, "end_ms": 7500},
        {"turn_index": 3, "speaker": "spk1", "text": "大概三天了，一开始只是轻微的，现在越来越严重。", "start_ms": 8000, "end_ms": 11000},
        {"turn_index": 4, "speaker": "spk0", "text": "有没有恶心、呕吐的症状？", "start_ms": 11500, "end_ms": 13000},
        {"turn_index": 5, "speaker": "spk1", "text": "有时候会感到恶心，但没有呕吐。", "start_ms": 13500, "end_ms": 15500},
        {"turn_index": 6, "speaker": "spk0", "text": "血压一百四十五九十五，有点偏高。我给你开点降压药，注意休息，少吃咸的食物。", "start_ms": 16000, "end_ms": 20000},
        {"turn_index": 7, "speaker": "spk1", "text": "好的，还需要注意什么吗？", "start_ms": 20500, "end_ms": 21500},
        {"turn_index": 8, "speaker": "spk0", "text": "一周后复查，如果头疼加重及时就诊。", "start_ms": 22000, "end_ms": 24000},
    ]
    
    for turn_data in turns_data:
        turn = TranscriptTurn(
            visit_id="perf_test_001",
            **turn_data
        )
        db.add(turn)
    
    db.commit()
    print("✅ 测试数据创建成功")
    return visit


def test_performance():
    """测试性能优化效果"""
    print("=" * 60)
    print("病历生成流程性能优化测试")
    print("=" * 60)
    
    init_db()
    db = SessionLocal()
    
    try:
        visit = db.query(Visit).filter(Visit.visit_id == "perf_test_001").first()
        if not visit:
            visit = create_test_data(db)
        
        print(f"\n📋 测试数据:")
        print(f"就诊ID: {visit.visit_id}")
        print(f"患者姓名: {visit.patient_name}")
        
        turns = db.query(TranscriptTurn).filter(
            TranscriptTurn.visit_id == visit.visit_id
        ).order_by(TranscriptTurn.turn_index).all()
        
        print(f"对话轮次: {len(turns)}")
        
        llm_config = db.query(LLMConfig).filter(LLMConfig.is_active == True).first()
        llm_service = LLMService(db) if llm_config else None
        
        if llm_service:
            print("\n✅ LLM服务已配置")
        else:
            print("\n⚠️  LLM服务未配置，将使用规则方法")
        
        pipeline = MedicalRecordPipeline(db, llm_service)
        
        print("\n" + "=" * 60)
        print("测试1: 串行模式")
        print("=" * 60)
        
        start_time = time.time()
        result_serial = pipeline.process_visit(
            visit.visit_id,
            use_llm=(llm_service is not None),
            save_intermediate=False,
            use_parallel=False
        )
        serial_time = time.time() - start_time
        
        print(f"\n📊 串行模式结果:")
        print(f"状态: {result_serial['status']}")
        print(f"总耗时: {serial_time:.2f}秒")
        print(f"证据数量: {result_serial['evidence_count']}")
        print(f"规范化术语数量: {result_serial['normalized_terms_count']}")
        print(f"抽取要素数量: {result_serial['extracted_items_count']}")
        print(f"跳过的步骤: {result_serial.get('skipped_steps', [])}")
        
        print("\n" + "=" * 60)
        print("测试2: 并行模式")
        print("=" * 60)
        
        start_time = time.time()
        result_parallel = pipeline.process_visit(
            visit.visit_id,
            use_llm=(llm_service is not None),
            save_intermediate=False,
            use_parallel=True
        )
        parallel_time = time.time() - start_time
        
        print(f"\n📊 并行模式结果:")
        print(f"状态: {result_parallel['status']}")
        print(f"总耗时: {parallel_time:.2f}秒")
        print(f"证据数量: {result_parallel['evidence_count']}")
        print(f"规范化术语数量: {result_parallel['normalized_terms_count']}")
        print(f"抽取要素数量: {result_parallel['extracted_items_count']}")
        print(f"跳过的步骤: {result_parallel.get('skipped_steps', [])}")
        
        print("\n" + "=" * 60)
        print("性能对比")
        print("=" * 60)
        print(f"串行模式耗时: {serial_time:.2f}秒")
        print(f"并行模式耗时: {parallel_time:.2f}秒")
        
        if parallel_time > 0:
            speedup = serial_time / parallel_time
            improvement = (serial_time - parallel_time) / serial_time * 100
            print(f"性能提升: {speedup:.2f}x")
            print(f"耗时减少: {improvement:.1f}%")
        
        print("\n" + "=" * 60)
        print("✅ 性能测试完成")
        print("=" * 60)
        
        return {
            "serial_time": serial_time,
            "parallel_time": parallel_time,
            "speedup": serial_time / parallel_time if parallel_time > 0 else 0,
            "improvement": (serial_time - parallel_time) / serial_time * 100 if serial_time > 0 else 0
        }
        
    except Exception as e:
        print(f"\n❌ 测试失败: {e}")
        import traceback
        traceback.print_exc()
        return None
    finally:
        db.close()


if __name__ == "__main__":
    result = test_performance()
    
    if result:
        print("\n" + "=" * 60)
        print("测试总结")
        print("=" * 60)
        print(f"串行模式: {result['serial_time']:.2f}秒")
        print(f"并行模式: {result['parallel_time']:.2f}秒")
        print(f"性能提升: {result['speedup']:.2f}x ({result['improvement']:.1f}% 减少)")
