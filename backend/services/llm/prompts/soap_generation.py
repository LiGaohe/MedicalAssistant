...
2. 否定性陈述（如"无XX"、"否认XX"）必须在对话中有患者明确否认或医生明确排除的原文依据，不得自行编造否定结论
3. **必须记录否定性事实**：当对话中出现明确的否定陈述时（如"没有去过医院"、"未做检查"、"没有过敏史"），必须以否定形式记录到对应字段中（如past_history记录"未去医院检查"、denied_symptoms记录"否认过敏史"），不得跳过或遗漏
4. 药物名称、剂量、规格必须在对话中有明确提及，不得编造具体数值或规格
5. 诊断必须在对话中有医生明确表述，不得自行推断诊断结论或升级诊断确定性
6. 如果对话中某信息不确定或未提及，对应字段留空，不要猜测
7. **完整性要求**：以下信息类型如果对话中有提及，必须记录到对应字段，不得遗漏：
   - 既往就诊次数、用药名称及疗效评价 → history_present_illness 或 past_history
   - 体温数值和发热持续时间 → history_present_illness
   - 呕吐/腹泻的具体次数和性状描述 → history_present_illness
   - 否定性事实（否认过敏史、检查结果正常等）→ denied_symptoms 或对应字段
   - 外院/既往检查结果 → auxiliary_examination 或 physical_examination
   - 进食/饮食变化情况 → history_present_illness
   - 家庭/环境因素（如交叉感染、免疫力差等）→ past_history
...
7. **必须记录否定性事实**：当对话中出现明确的否定陈述时（如"没有去过医院"、"未做检查"、"没有过敏史"），必须以否定形式记录到对应段落中（如S段记录"未去医院检查"、"否认过敏史"），不得跳过或遗漏
8. 药物名称、剂量、规格必须在对话中有明确提及，不得编造具体数值或规格
9. 诊断结论必须在对话中有医生明确表述，不得自行推断或升级诊断确定性（如"考虑XX"不能改为"确诊XX"）
10. **完整性要求**：以下信息类型如果对话中有提及，必须记录，不得遗漏：
    - 既往就诊次数、用药名称及疗效评价
    - 体温数值和发热持续时间
    - 呕吐/腹泻的具体次数和性状描述
    - 否定性事实（否认过敏史、检查结果正常等）
...
5. **必须保留否定性事实**：如果草稿中包含否定性事实（如"未去医院检查"、"否认过敏史"），结构化时必须保留到对应字段中（如past_history记录"未去医院检查"、denied_symptoms记录"否认过敏史"），不得跳过或遗漏
6. 药物、检查的具体数值和规格必须与草稿文本一致，不得添加或编造
