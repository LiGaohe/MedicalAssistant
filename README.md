...
将门诊医患对话音频自动转化为结构化电子病历（EMR）的系统：语音识别转写 → 说话人分离 → LLM 生成 SOAP 病历 → 术语规范化 → 幻觉检查 → 字段级修订，全程支持证据溯源与量化评测。
...
### 音频到文本（M1）
- 音频上传与 ASR 转写，支持说话人分离、时间戳与置信度输出
- 医疗术语智能纠错，支持集成对比多个 ASR 引擎

### 文本到病历（M2）
- 四阶段生成管线：证据选择 → 术语规范化 → 要素抽取（SOAP 格式）→ 结构化病历生成
- 证据回链：每个病历字段可追溯到原始对话
- 多版本病历管理

### 质量保障（M3/M4）
- 幻觉检查：逐章节核查病历内容是否有对话依据
- 后置核查与字段级修订：遗漏检查、矛盾检测、最小化改动修订
- 评测：对转写、术语归一化、字段抽取进行量化评估与消融实验
...
# 关闭端口8000的进程
# Windows:
netstat -ano | findstr 8000 # 查找端口8000的进程id，例如：22384
powershell -Command "Stop-Process -Id 22384 -Force"
# 如果还清理不掉
